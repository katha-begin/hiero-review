...
class ReviewToolController(QObject):
    ...
    def _on_build_timeline(self, config: dict) -> None:
        ...
        try:
            ...
            # Get all available sequences for this episode (to detect "all" selection)
            all_sequences = self.scanner.scan_sequences(config['episode']) if self.scanner else []

            # Build TimelineConfig (name is generated based on sequence selection)
            timeline_config = TimelineConfig(
                name="",  # Will be generated by generate_timeline_name()
            ...
            # Log the generated timeline name
            timeline_name = timeline_config.generate_timeline_name()
            self.dialog.log_message(f"Timeline: {timeline_name}", "info")
            ...
            if result.success:
                self.dialog.log_message("=" * 50, "info")
                if result.is_update:
                    self.dialog.log_message(f"SUCCESS: Timeline '{timeline_name}' updated!", "info")
                    self.dialog.log_message(f"Shots added: {result.shots_added}", "info")
                    self.dialog.log_message(f"Shots updated: {result.shots_updated}", "info")
                else:
